{
  ...
  function RenderAndEmail() {
    ...
    if (safeToRunScript) {
      ...
      if (
        !settings.haveSetting("Email Settings", "Mail Server") ||
      ...
      if (
        ...
        alert(
          "Can't send email; I don't have all the settings I need. Aborting.",
          scriptName
        );
      } else {
        try {
          // Send the email.
          var serverSetting = settings.getSetting(
            "Email Settings",
            "Mail Server"
          );
          var fromSetting = settings.getSetting(
            "Email Settings",
            "Reply-to Address"
          );
          var toSetting = settings.getSetting(
            "Email Settings",
            "Render Report Recipient"
          );
          var authUser;
          var authPass;
